...
class ResNet_s(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes=10, use_norm=False):
        ...
        if use_norm:
            ...
            self.linear_b = NormedLinear(32, num_classes)
        else:
            self.linear = nn.Linear(64, num_classes)
            self.linear_b = nn.Linear(32, num_classes)
    ...
    def forward(self, x):
        ...
        out = self.linear(out)
        b = F.avg_pool2d(b, b.size()[2:])      # Global average pooling: [batch_size, channels, 1, 1]
        b = b.view(b.size(0), -1)             # Flatten: [batch_size, channels]
        b = F.normalize(b, dim=1)             # Normalize: [batch_size, channels]
        b = b.unsqueeze(1) 
        # print(b.size())
        return out,b


def resnet20(num_classes,use_norm=False):
    return ResNet_s(BasicBlock, [3, 3, 3],num_classes=num_classes,use_norm=use_norm)


def resnet32(num_classes, use_norm=False):
    return ResNet_s(BasicBlock, [5, 5, 5], num_classes=num_classes, use_norm=use_norm)


def resnet44(num_classes,use_norm=False):
    return ResNet_s(BasicBlock, [7, 7, 7],num_classes=num_classes,use_norm=use_norm)


def resnet56(num_classes,use_norm=False):
    return ResNet_s(BasicBlock, [9, 9, 9],num_classes=num_classes,use_norm=use_norm)


def resnet110(num_classes,use_norm=False):
    return ResNet_s(BasicBlock, [18, 18, 18],num_classes=num_classes,use_norm=use_norm)


def resnet1202(num_classes,use_norm=False):
